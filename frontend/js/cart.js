...
export async function initCart() {
  ...
  // Încărcăm coșul din localStorage
  loadCartFromStorage();

  // Încărcăm configurația Stripe cu retry logic
  let retries = 0;
  const maxRetries = 3;

  while (retries < maxRetries && !stripe) {
    try {
      console.log(
        `🔄 Încercare ${retries + 1}/${maxRetries} de inițializare Stripe...`
      );
      stripePublishableKey = await getStripeConfig();

      if (!stripePublishableKey) {
        throw new Error("Publishable key lipsește");
      }

      // Verificăm că Stripe SDK e încărcat
      if (typeof Stripe === "undefined") {
        throw new Error("Stripe SDK nu este încărcat. Verifică index.html!");
      }
      ...
      console.error(`❌ Încercare ${retries + 1} eșuată:`, error);
      retries++;

      if (retries < maxRetries) {
        // Așteptăm 1 secundă înainte de retry
        await new Promise((resolve) => setTimeout(resolve, 1000));
      } else {
        console.error("❌ Nu s-a putut inițializa Stripe după 3 încercări");
        alert(
          "⚠️ Eroare: Nu se poate conecta la Stripe. Verifică:\n1. Fișierul .env are STRIPE_PUBLISHABLE_KEY\n2. Serverul backend rulează\n3. Console (F12) pentru erori"
        );
      }
    }
  }
